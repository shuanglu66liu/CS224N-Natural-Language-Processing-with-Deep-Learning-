...
class NERModel(LanguageModel):
  ...
  def add_model(self, window):
    ...
    with tf.variable_scope("Layer"):
       W = tf.get_variable("W", shape=[self.config.window_size*self.config.embed_size, self.config.hidden_size],initializer=xavier_weight_init())
    ...
    y_hat=tf.matmul(h,U)+b2
    y_hat = tf.nn.dropout(y_hat, self.dropout_placeholder)
