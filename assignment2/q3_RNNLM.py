...
class RNNLM_Model(LanguageModel):
  ...
  def add_embedding(self):
    ...
    with tf.device('/cpu:0'):
      ...
      embed_list=tf.split(embed,self.config.num_steps,1)
      inputs=[tf.squeeze(i) for i in embed_list ]
      #raise NotImplementedError
      ### END YOUR CODE
      return inputs
  ...
  def add_projection(self, rnn_outputs):
    ...
    with tf.variable_scope("Softmax"):
       U = tf.get_variable("U", shape=(self.config.hidden_size,len(self.vocab)),dtype=tf.float32)
       b_2 = tf.get_variable("b_2", shape=(len(self.vocab),),dtype=tf.float32) 
    outputs=[tf.matmul(U,item)+b_2 for item in  rnn_outputs]
    #raise NotImplementedError
    ### END YOUR CODE
    return outputs
  ...
  def add_loss_op(self, output):
    ...
    loss=sequence_loss(output,
                       tf.to_int32(tf.reshape(
                             self.labels_placeholder,
                             [self.config.batch_size * self.config.num_steps, -1])),
                       tf.ones(shape=(self.config.batch_size, self.config.num_steps)))
    #raise NotImplementedError
    ### END YOUR CODE
    return loss
  ...
  def add_model(self, inputs):
    ...
    with tf.variable_scope("RNN") as scope:
       ...
       H = tf.get_variable("H", shape=(self.config.hidden_size,self.config.hidden_size),dtype=tf.float32)
       I = tf.get_variable("I", shape=(self.config.embed_size,self.config.hidden_size),dtype=tf.float32)
       b_1 = tf.get_variable("b_1", shape=(self.config.hidden_size,),dtype=tf.float32) 
       
       for step in range(self.config.num_steps):
           if step ==0:
             h_next=tf.matmul(self.initial_state,H)+tf.matmul(inputs[step],I)+b_1
             rnn_outputs.append(h_next)
             h_prev=h_next
           else:
             h_next=tf.matmul(h_prev,H)+tf.matmul(inputs[step],I)+b_1
             rnn_outputs.append(h_next)
             h_prev=h_next 
               
    self.final_state=h_next
    #raise NotImplementedError
    ### END YOUR CODE
    return rnn_outputs
