...
class SoftmaxModel(Model):
  ...
  def add_model(self, input_data):
    ...
    W = tf.get_variable("weights", dtype=tf.float32, shape=(self.config.n_features,self.config.n_classes),initializer=tf.zeros_initializer)
    b=tf.get_variable("bias", dtype=tf.float32, shape=(self.config.n_classes,),initializer=tf.zeros_initializer)
