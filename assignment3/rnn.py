...
class RNN_Model():
    ...
    def add_model_vars(self):
        ...
        with tf.variable_scope('Composition'):
            ### YOUR CODE HERE
            embedding = tf.Variable('embedding',
            tf.random_uniform([len(self.vocab), self.config.embed_size], -1.0, 1.0))
            W1 = tf.get_variable("W1", shape=(2*self.config.embed_size,self.config.embed_size),dtype=tf.float32)
            b1=tf.get_variable("b1", shape=(1,self.config.embed_size),dtype=tf.float32)
            ### END YOUR CODE
        with tf.variable_scope('Projection'):
            ### YOUR CODE HERE
            U = tf.get_variable("U", shape=(self.config.embed_size,self.config.label_size),dtype=tf.float32)
            bs=tf.get_variable("bs", shape=(1,self.config.label_size),dtype=tf.float32)
            ### END YOUR CODE
    ...
    def add_model(self, node):
        ...
        if node.isLeaf:
            ...
            ### YOUR CODE HERE
            curr_node_tensor=tf.nn.relu(tf.matmul(tf.concat(node_tensors[node.left],node_tensors[node.right]),W1)+b1)
            ### END YOUR CODE
    ...
    def loss(self, logits, labels):
        ...
        loss=loss+self.config.l2*tf.get_collection("l2_loss")[0]
        # END YOUR CODE
        return loss
